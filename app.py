...
def init_and_migrate_db():
    ...
    if USE_POSTGRES:
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS candidates_data (
        ...
        ''')
        cursor.execute("PRAGMA table_info(candidates_data)")
        columns = [col[1] for col in cursor.fetchall()]
        
        if "agencia" not in columns:
            cursor.execute("ALTER TABLE candidates_data ADD COLUMN agencia TEXT DEFAULT 'General Agency'")
        if "hospital_preferido" not in columns:
            cursor.execute("ALTER TABLE candidates_data ADD COLUMN hospital_preferido TEXT")
        if "doctor_preferido" not in columns:
            cursor.execute("ALTER TABLE candidates_data ADD COLUMN doctor_preferido TEXT")
        if "monday_item_id" not in columns:
            cursor.execute("ALTER TABLE candidates_data ADD COLUMN monday_item_id TEXT")
        if "enrollment_status" not in columns:
            cursor.execute("ALTER TABLE candidates_data ADD COLUMN enrollment_status TEXT DEFAULT 'Pending'")
...
def get_clean_agencies():
    conn = get_db_connection()
    df = pd.read_sql_query("SELECT DISTINCT agencia FROM candidates_data WHERE agencia IS NOT NULL AND agencia != ''", conn)
    conn.close()
    if df.empty: return []
...
def generate_exact_quote_pdf(candidate_name, hospital_pref, obgyn_pref, hosp_in_net, doc_in_net, pregnant, current_plan, plans):
    ...
    story.append(Paragraph(f"Hello,<br/><br/>Thank you for submitting <b>{candidate_name}'s</b> enrollment form.", style_normal))
...
if menu_option == "🔍 Search & Quote Candidate":
    ...
    if selected_agency == "All Agencies":
        cursor.execute("SELECT nombre_gc FROM candidates_data ORDER BY nombre_gc ASC")
    else:
        cursor.execute("SELECT nombre_gc FROM candidates_data WHERE TRIM(agencia) LIKE ? ORDER BY nombre_gc ASC", (f"%{selected_agency}%",))
    ...
    if selected_candidate:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id_cliente, edad, estado, codigo_postal, compania_seguros, hospital_preferido, doctor_preferido, agencia FROM candidates_data WHERE nombre_gc = ?", (selected_candidate,))
        row = cursor.fetchone()
        conn.close()
            ...
            with quote_builder:
                ...
                if submitted and final_plans:
                    ...
                    try:
                        conn = get_db_connection()
                        cursor = conn.cursor()
                        cursor.execute("SELECT monday_item_id FROM candidates_data WHERE nombre_gc = ?", (selected_candidate,))
                        ...
                        if item_id_row and item_id_row[0]:
                            ...
                            if push_status_to_monday(item_id, "Quotes Sent"):
                                cursor.execute("UPDATE candidates_data SET enrollment_status = 'Quotes Sent' WHERE id_cliente = ?", (id_c,))
                                conn.commit()
...
# --- 10. NUEVO MÓDULO CRM ---
    ...
    st.title("Pending Enrollments (Operations View)")
            ...
            if orig_status != new_status:
                ...
                with st.spinner(f"Updating {row['Candidate']} in monday.com..."):
                    if push_status_to_monday(item_id, new_status):
                        conn = get_db_connection()
                        conn.cursor().execute("UPDATE candidates_data SET enrollment_status = ? WHERE monday_item_id = ?", (new_status, item_id))
                        conn.commit()
                        conn.close()
                        changes += 1
                    else:
    ...
    with st.form("form_add_new"):
        ...
        if st.form_submit_button("Save Candidate", use_container_width=True) and name and zip_in:
            ...
            try:
                conn.cursor().execute('''
                    INSERT INTO candidates_data (nombre_gc, edad, estado, codigo_postal, compania_seguros, agencia, hospital_preferido, doctor_preferido)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    conn = get_db_connection()
    df_hist = pd.read_sql_query('SELECT nombre_gc AS Candidate, agencia AS Agency, edad AS Age, estado AS State, codigo_postal AS Zip, compania_seguros AS Carrier, enrollment_status AS Status FROM candidates_data', conn)
    conn.close()
