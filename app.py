...
def buscar_logo():
    # In public demo version, logo is omitted for privacy and clean styling
    return None
...
st.set_page_config(
    ...
    page_title="Health Insurance Quotes Portal (Demo)",
...
def make_hash(password):
    ...
    return hashlib.pbkdf2_hmac("sha256", str(password).strip().encode("utf-8"), b"quotes-demo-salt", 200_000).hex()
...
DEFAULT_USERS = {
    "demo": [
        "58d3fe2e6f0f2cd603136b54cbd630385cbc3b3f5a18d47353f5f453a26eac9a",  # Demo2026!
    ],
    "admin": [
        "58d3fe2e6f0f2cd603136b54cbd630385cbc3b3f5a18d47353f5f453a26eac9a",  # Demo2026!
    ],
...
if not st.session_state["authenticated"]:
    ...
    if logo_b64:
        ...
        st.markdown(f'<div class="login-logo-container"><img src="data:image/png;base64,{logo_b64}" alt="Quotes Portal Logo"></div>', unsafe_allow_html=True)

    else:

        st.markdown('<div class="login-logo-container"><h1 style="color:#161C63; font-weight:800; font-size:2.2rem; letter-spacing:-0.5px;">Quotes Portal</h1></div>', unsafe_allow_html=True)
    ...
    with form_col:
        ...
        st.markdown('<div class="login-footer-secure"><span>🔒</span> <span>Quotes Portal (Demo)</span></div>', unsafe_allow_html=True)
...
def generar_cotizacion_pdf(candidate_name, agency_name, hospital_pref, obgyn_pref, hosp_in_net, doc_in_net, pregnant, current_plan, plans):
    ...
    # SECCIÓN 2: TABLA RESUMEN EJECUTIVO
    ...
    # Filas exactas del resumen ejecutivo
...
def mostrar_pantalla_carga(user_name="User"):
    ...
    logo_html = f'<img src="data:image/png;base64,{logo_b64}" style="height:72px; max-width:240px; object-fit:contain; margin-bottom:1.4rem; filter:drop-shadow(0 4px 12px rgba(235, 60, 150, 0.3));" />' if logo_b64 else '<h1 style="color:#FFFFFF; font-weight:800; margin:0 0 1.4rem 0; letter-spacing:-0.5px;">Quotes Portal</h1>'
...
<span style="font-weight:700; color:#0F172A;">{plan.get('rx', '$15 copay')}</span>
            ...
            if len(f_plans) > 0:
                ...
                file_name = f"Quote_Proposal_{clean_cand_fn}.pdf"
...
# ==============================================================================
    ...
    st.title("Pending Enrollments (Broker View)")
...
# ==============================================================================
    ...
    with st.form("form_add_new"):
        ...
        agency_in = st.text_input("Agency / Organization:", "General Agency", placeholder="e.g. Pacific Health Agency")
        ...
        hosp_in   = st.text_input("Preferred Hospital (Optional):", placeholder="e.g. Cedars-Sinai Medical Center")

        doc_in    = st.text_input("Preferred OB-GYN (Optional):", placeholder="e.g. Dr. Sarah Connor")
